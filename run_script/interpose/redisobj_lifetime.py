...
class ZFamCaller: # Caller of zmalloc family
    def __init__(self, call_cnt, obj_list, total_msize, total_rcnt, total_wcnt, avg_msize, avg_rcnt, avg_wcnt, avg_cnt):
        self.call_cnt = call_cnt
        self.obj_list = obj_list # a list of mem_entries. 
        self.total_msize = total_msize
        self.total_rcnt = total_rcnt # total read cnt of all rw_entries in the obj_list
        self.total_wcnt = total_wcnt # total write cnt of all rw_entries in the obj_list
        self.avg_msize = avg_msize
        self.avg_rcnt = avg_rcnt
        self.avg_wcnt = avg_wcnt
        self.avg_cnt = avg_cnt
...
def open_pot_file(input_file_name, num_objs):
    if input_file_name[-5:] != ".vout":
        print("input file [%s] is not .vout!" % (input_file_name))
        exit()
    pot_file_name = input_file_name[:-5] + ".pot"
    try:
        pot_file = open(pot_file_name, 'w')
    except:
        sys.stderr.write("File write failed: %s\n" % pot_file_name)
        exit(1)

    trace_meta_str = "{0}\n".format(num_objs)
    pot_file.write(trace_meta_str)

    return pot_file

def generate_pot(pot, mentry):
    """ # Generate and save per-object trace (.pot)
            <trace metadata> numobjs, ...
                objid objsize objaddr objrcnt objwcnt 
                <obj metadata>: # of lines, ...
                    rwtype rwaddr rwtimestamp funcname funcid 
                    rwtype rwaddr rwtimestamp funcname funcid 
                    rwtype rwaddr rwtimestamp funcname funcid 
                    ...
    """
    ...
    objstr = "{0} {1} {2:x} {3} {4}\n".format(
               mentry.malloc_id, mentry.msize, mentry.maddr, mentry.rcnt, mentry.wcnt)
    pot.write(objstr)

    metastr = "{0}\n".format(len(mentry.line_group))
    pot.write(metastr)

    for rw_entry in mentry.line_group:
        rwstr = "{0} {1:x} {2} {3} {4}\n".format(
                rw_entry.rw_type, rw_entry.rw_addr, rw_entry.rw_timestamp,
                rw_entry.func_name, rw_entry.func_id)
        pot.write(rwstr)
    return
...
def interprete_line(line_type, line):
    ...
    global output_file
    global g_zcaller, g_zcaller_name, zcaller_dict, zfamily_switch
    ...
    if line_type == LINE_FUNC_BEGIN:
        ...
        if zfamily_switch == 1:
            zfamily_switch = 0
            #g_zcaller.obj_list.append(malloc_entry)
            zcaller_dict[g_zcaller_name].obj_list.append(malloc_entry)
            #print("g_zcaller_name:", g_zcaller_name)
            #print("obj_list:", zcaller_dict[g_zcaller_name].obj_list)
            #print("meet zfamily->malloc! mentry:", malloc_entry)
            #g_zcaller.total_msize += msize
        ...
        if faddr in opened_mallocs.keys: # opened_mallocs이 커서 이렇게 찾는 게 별로. 이것도 bs로 바꾸자.
            ...
            #print("line: %s" % (line))
        """
    elif line_type == LINE_ZFAMILY_CALL:
        splitline = line.replace("\n", "").split(" ")
        zfamily_type = int(splitline[2], 10)
        zcaller_name = splitline[3] 
        zfamily_switch = 1 # global
        g_zcaller_name = zcaller_name
        if not zcaller_name in zcaller_dict:
            zfam_caller = ZFamCaller(call_cnt=0, obj_list=[], total_msize=0, total_rcnt=0, total_wcnt=0, avg_msize=0, avg_rcnt=0, avg_wcnt=0, avg_cnt=0)
            zcaller_dict[zcaller_name] = zfam_caller
        g_zcaller = zcaller_dict[zcaller_name] # global
        g_zcaller.call_cnt += 1
...
def zfam_analyze():
    ...
    ###
    rcnt_to_obj = 0; wcnt_to_obj = 0

    # count rcnt/wcnt (code block from analyze_object()
    ...
    for mid, mentry in malloc_dict.items():
        ...
        rcnt_to_obj += rcnt_obj
        wcnt_to_obj += wcnt_obj
    ###
    cnt_to_obj = rcnt_to_obj + wcnt_to_obj
    print("RW to objects: %d (R %d / W %d)" % (cnt_to_obj, rcnt_to_obj, wcnt_to_obj))

    for zcaller_name, zcaller in zcaller_dict.items():
        if len(zcaller.obj_list) == 0:
            continue
        for mentry in zcaller.obj_list:
            zcaller.total_msize += mentry.msize
            zcaller.total_rcnt += mentry.rcnt
            zcaller.total_wcnt += mentry.wcnt
        zcaller.avg_msize = zcaller.total_msize / len(zcaller.obj_list)
        zcaller.avg_rcnt = zcaller.total_rcnt / len(zcaller.obj_list)
        zcaller.avg_wcnt = zcaller.total_wcnt / len(zcaller.obj_list)
        zcaller.avg_cnt = (zcaller.total_rcnt+zcaller.total_wcnt) / len(zcaller.obj_list)

    for zcaller_name, zcaller in zcaller_dict.items():
        print("%32s | call_cnt: %8d, msize: avg %7.1f (total %8d), rcnt: avg %7.1f (total %8d), wcnt: avg %7.1f (total %8d), cnt: avg %7.1f (total %8d)" 
                % (zcaller_name, zcaller.call_cnt, zcaller.avg_msize, zcaller.total_msize,
                    zcaller.avg_rcnt, zcaller.total_rcnt, zcaller.avg_wcnt, zcaller.total_wcnt,
                    zcaller.avg_cnt, zcaller.total_rcnt+zcaller.total_wcnt))

def analyze_object(input_file_name):
    global malloc_dict
    global popf_dict
    global output_file
    ...
    pot_file = open_pot_file(input_file_name, num_objs)
    ...
    for mid, mentry in malloc_dict.items():
        ...
        for rw_entry in mentry.line_group:
            ...
            # 여기서, 그냥 mentry.addrset을 만들어서 바로 metric에 넣어도 된다. 
            # 일단은, .pout을 보고 그냥 mentry를 새로 만들기로 하자.
...
def main():
    ...
    #analyze_object(input_file_name) # 기존 작업도 하게 되면서 메모리를 많이 사용함. 하지만 여기 안에 zfam_analyze에 필요한 rcnt/wcnt 측정 작업도 있어서 이 함수를 호출하지 않는다면 mentry.rcnt/wcnt는 따로 만들어줘야 함
