...
class HashSet:
# list에서 이걸로 바꾸면, func begin은 길어지고, func end는 짧아졌다.
# 공통적으로 있는 if a in opened_func는, set이 빠를 것이다. list는 O(n), set은 O(1).
# 1은 append, 2는 remove인데, set append가 느린가?
# -> list append는 그냥 추가하면 되지만, list remove는 다 shift해야 한다.
# -> set append는 hash lookup만 하고 추가하면 되지만, set remove는 그냥 지우면 된다.
    def __init__(self):
        self.set = set()
...
block1_time=0; block2_time=0; block3_time=0; block4_time=0; block5_time=0;
...
def interprete_line(line_type, line):
    ...
    global output_file
    global block1_time, block2_time, block3_time, block4_time, block5_time
    ...
    if line_type == LINE_FUNC_BEGIN:
        ...
        if faddr in opened_mallocs.keys: # opened_mallocs이 커서 이렇게 찾는 게 별로. 이것도 bs로 바꾸자.
            ...
            #print("line: %s" % (line))
        """
        block4_time = time.time() - start_time
        ...
        block5_time = time.time() - start_time
        """
        i=0
...
def read_trace_file(linenum, raw_trace_file):
    ...
    while True:
        ...
        if line_type == LINE_FUNC_BEGIN:
            block1_total_time += block1_time
        elif line_type == LINE_FUNC_END:
            block2_total_time += block2_time
        elif line_type == LINE_MALLOC_CALL:
            block3_total_time += block3_time
        elif line_type == LINE_FREE_CALL:
            block4_total_time += block4_time
        elif line_type == LINE_RW:
            block5_total_time += block5_time
    ...
    print("block_total_time: %9f/%9f/%9f/%9f/%9f" % 
            (block1_total_time,block2_total_time,block3_total_time,block4_total_time,block5_total_time))
